...
try:
    ...
    try:
        ...
        sys_compatibility.patch_unix_modules()
        logging.info("通过备选路径应用系统兼容性补丁")
    except Exception as inner_e:
        logging.warning(f"通过备选路径应用系统兼容性补丁失败: {str(inner_e)}")
...
from vectorizer import get_vectorizer, SimpleMemoryVectorStore
...
if app_mode == "RAG问答系统":
    ...
    with tab1:
        ...
        if not st.session_state.messages:
            st.info("👋 欢迎使用RAG问答系统！请先上传并索引文档，然后在此输入您的问题。")
            with st.expander("💡 使用指南", expanded=True):
                guide_text = """
                1. 在"📚 知识库管理"选项卡中上传您的文档
                2. 点击"处理并索引文档"按钮创建向量索引
                3. 回到此选项卡，开始提问
                
                系统将基于您上传的文档生成回答，并提供引用来源。
                """
        ...
        if not st.session_state.retriever:
            try:
                ...
                if vector_store is None:
                    if not st.session_state.messages:  # 只在初始状态显示一次
                        st.warning("⚠️ 知识库尚未创建，请先上传并索引文档。切换到'📚 知识库管理'选项卡来上传文档。")
                elif isinstance(vector_store, SimpleMemoryVectorStore) and len(vector_store.documents) == 0:
                    # 是空的内存向量存储，表示没有实际索引
                    st.info("📝 系统已启动但知识库为空。请在'📚 知识库管理'选项卡上传并索引文档，以启用更好的问答能力。")
                    
                    # 仍然初始化检索器，但提醒用户这是空的知识库
                    ...
                    st.success("✅ 系统已初始化! 可以开始提问，但回答将不基于文档知识。建议上传文档以获得更好的回答。")
                else:
                    # 初始化检索器
                    st.session_state.retriever = get_retriever(
                        vector_store=vector_store,
            ...
            try:
                ...
                if st.session_state.retriever:
                    try:
                        ...
                        st.success("✅ RAG生成器已成功初始化！您可以使用文档知识进行问答。")
                    except Exception as e:
                        st.error(f"初始化RAG生成器失败: {str(e)}")
                        st.warning("尝试初始化基本生成器...")
                        
                        # 尝试初始化一个基本的生成器，不依赖检索器
                        try:
                            st.session_state.generator = get_generator(
                                retriever=None,
                                model_name=model_name,
                            ...
                            st.info("📝 已初始化基本生成器（无文档增强）。您可以开始对话，但回答将不基于知识库。")
                        except Exception as inner_e:
                            st.error(f"初始化基本生成器也失败: {str(inner_e)}")
                    ...
                    try:
                        ...
                        st.info("📝 已初始化基本生成器（无文档增强）。您可以开始对话，但回答将不基于知识库。建议上传文档以获得更好的回答。")
                    except Exception as e:
                        st.error(f"初始化基本生成器失败: {str(e)}")
                        st.warning("请检查API密钥是否正确配置。")
            except Exception as e:
        ...
        if prompt := st.chat_input("请输入您的问题"):
            ...
            with st.chat_message("assistant"):
                ...
                elif st.session_state.generator:
                    # 只有生成器但没有检索器的情况
                    try:
                        message_placeholder = st.empty()
                    ...
                    vector_store = vectorizer.create_vector_store(chunks)
                            if vector_store is None:
                                ...
                                try:
                                    ...
                                    st.success("成功创建内存向量存储作为备选")
                                except Exception as backup_err:
                                    st.error(f"创建备选向量存储也失败: {str(backup_err)}")
                                    st.warning("应用将以有限功能继续运行，但检索功能可能不可用")
                            else:
                                st.success("成功创建向量存储")
                        except Exception as e:
                            st.error(f"创建向量存储时出错: {str(e)}")
                            st.warning("尝试降级到基本功能模式，某些功能可能不可用")
